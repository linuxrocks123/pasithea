...
PROGRAMS = ("philotes","hypnos",)#"pasithea")
FLAGS="-g"
...
for fname in flist:
    if has_suffix(fname,".y"):
        ...
        print base_name+".h bison --defines="+base_name+".h "+base_name+".y"
        print base_name+".tab.c bison --defines="+base_name+".h "+base_name+".y"
        if LINUX:
            print base_name+".tab.o g++ -std=gnu++1y -c "+FLAGS+" "+base_name+".tab.c"
        if WIN32:
            print base_name+".tab.obj /opt/mxe/usr/bin/i686-w64-mingw32.static-g++ -std=gnu++1y -static -c "+FLAGS+" "+base_name+".tab.c -o "+base_name+".tab.obj"
        if MACOSX:
            print base_name+'.tab.macho sh -c "source ./mac.source; o64-g++ -I/opt/osxcross/target/macports/pkgs/opt/local/include -std=gnu++1y -c '+FLAGS+' '+base_name+'.tab.c -o '+base_name+'.tab.macho"'
        common_objects.append(base_name+".tab")
        for target in PROGRAMS:
            if LINUX:
                print base_name+".tab.o / "+target
            if WIN32:
                print base_name+".tab.obj / "+target+"_win32"
            if MACOSX:
        ...
        print "lex.yy.c flex "+base_name+".l"
        if LINUX:
        ...
        if LINUX:
            print "lex.yy.o g++ -std=gnu++1y -c "+FLAGS+" lex.yy.c"
        if WIN32:
            print "lex.yy.obj /opt/mxe/usr/bin/i686-w64-mingw32.static-g++ -std=gnu++1y -static -c "+FLAGS+" lex.yy.c -o lex.yy.obj"
        if MACOSX:
            print 'lex.yy.macho sh -c "source ./mac.source; o64-g++ -I/opt/osxcross/target/macports/pkgs/opt/local/include -std=gnu++1y -c '+FLAGS+' lex.yy.c -o lex.yy.macho"'
        common_objects.append("lex.yy")
        for target in PROGRAMS:
            if LINUX:
                print "lex.yy.o / "+target
            if WIN32:
                print "lex.yy.obj / "+target+"_win32"
            if MACOSX:
        ...
        if WIN32:
            print base_name+".obj /opt/mxe/usr/bin/i686-w64-mingw32.static-g++ -std=gnu++1y -static -c "+FLAGS+" "+base_name+".cpp -o "+base_name+".obj"
        if MACOSX:
        ...
        if not has_suffix(fname,"_main.cpp"):
            ...
            for target in PROGRAMS:
                ...
                if WIN32:
                    print base_name+".obj / "+target+"_win32"
                if MACOSX:
            ...
            if WIN32:
                print base_name+".obj / "+fname[:fname.index("_")]+"_win32"
            if MACOSX:
    ...
    ###THIS PART CUSTOM
        ...
        base_name = fname[:len(fname)-4]
        print base_name+".java / "+base_name+".xxd"
        print base_name+'.xxd sh -c "xxd -i < '+base_name+'.java > '+base_name+'.xxd"'

for target in PROGRAMS:
    if LINUX:
        print target+' sh -c "g++ -std=gnu++1y '+' '.join(map(lambda x: x+".o",common_objects))+' '+target+'_main.o -o '+target+'"'
    if WIN32:
        print target+'_win32 sh -c "/opt/mxe/usr/bin/i686-w64-mingw32.static-g++ -std=gnu++1y -static '+' '.join(map(lambda x: x+".obj",common_objects))+' '+target+'_main.obj /opt/mxe/usr/i686-w64-mingw32.static/lib/libfltk.a win32_bin/cyg*.dll -lws2_32 -lole32 -luuid -lcomctl32 -o '+target+'_win32"'
    if MACOSX:
